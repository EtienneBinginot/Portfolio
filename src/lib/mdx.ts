...
export async function compileProjectMdx(source: string, project: Project) {
  const { content } = matter(source);
  const result = await evaluate({
    source: content,
    components: buildMdxComponents(project),
...
// `writeup` du schéma).
export async function getProjectWriteup(project: Project, locale: Locale) {
  ...
  return compileProjectMdx(raw, project);
}
